...
class PhysicalPort(viewsets.GenericViewSet):
    serializer_class = Serializers.phyport
    queryset = models.PhysicalPort.objects.all()

    @grainy_endpoint(
        namespace = "phyport.{request.org.permission_id}"
    )
    def list(self, request, org, instance, *args, **kwargs):
        serializer = Serializers.phyport(
...
class LogicalPort(viewsets.GenericViewSet):
    serializer_class = Serializers.logport
    queryset = models.LogicalPort.objects.all()

    @grainy_endpoint(
        namespace = "logport.{request.org.permission_id}"
    )
    def list(self, request, org, instance, *args, **kwargs):
        serializer = Serializers.logport(
            instance.logport_set.all(),
            many=True,
        )
        return Response(serializer.data)
...
class VirtualPort(viewsets.GenericViewSet):
    serializer_class = Serializers.virtport
    queryset = models.VirtualPort.objects.all()

    @grainy_endpoint(
        namespace = "virtport.{request.org.permission_id}"
    )
    def list(self, request, org, instance, *args, **kwargs):
        serializer = Serializers.virtport(
            self.get_queryset().filter(logport__instance=instance),
            many=True,
        )
        return Response(serializer.data)
